...
### MediaX SDK 对齐说明
- 根据 `docs/plan/mediax-sdk.md`，SessionTokenClient 需要由 `cmd/sessiontoken/main.go` 启动的独立 HTTP 服务承载，默认监听 `:7070`，并加载配置后注册 `/session-token/flows` 路由；服务启动顺序与插件端 BaseURL/API Token/CallbackURL 必须通过 `POWERX_SESSION_TOKEN_*` 环境变量保持一致。
- HTTP 层必须复用 `pkg/server/handlers/session_token` 的 `RegisterSessionTokenFlowCreateRoute` 与 `RegisterSessionTokenFlowGetRoute`，并在中间件中校验 Bearer Token（默认 `dev-session-token`，可配置）。
- Flow 成功后要向插件传入的 `callback_url` （`/admin/platforms/session-token/callback`）回调标准化凭证，并输出 `sessiontoken_metric` 与 `sessiontoken_callback` 观测日志，确保 MediaX SDK 与插件共同诊断 SLA。
- 本 spec 仍以 SessionTokenClient 为核心，但需在非功能需求中描述 OAuth/AuthManager 复用点与频道刷新/任务接口的依赖，确保 MediaX SDK 的交互方式保持一致。
...
### User Story 5 - SessionToken 服务部署与配置共享 (Priority: P1)
MediaX 运维/插件团队需要快速在本地或 MediaXCore 中启动 SessionToken HTTP 服务，并确保与插件端 BaseURL、API Token、CallbackURL 完全一致，以降低联调成本。

**Why this priority**: 没有统一的服务进程、命令与配置模板，插件无法稳定访问 `/session-token/flows`，也无法通过 `POWERX_SESSION_TOKEN_*` 与 MediaX SDK 对齐。

**Independent Test**: 通过 `make sessiontoken` 或 `go run ./cmd/sessiontoken` 启动服务，命中 `POST/GET /session-token/flows`，并验证 `sessiontoken_metric` 日志与插件回调签名。

**Acceptance Scenarios**:

1. **Given** 运维执行 `make sessiontoken`，**When** 服务读取 `config.yaml`/环境中的 `session_token` 配置，**Then** 在 `:7070` 暴露路由并复用 `RegisterSessionTokenFlowCreateRoute`/`GetRoute`，可通过 Bearer token 访问。
2. **Given** 插件端提供 `POWERX_SESSION_TOKEN_BASE_URL`、`POWERX_SESSION_TOKEN_API_TOKEN` 与 `callback_url`，**When** Flow 完成，**Then** MediaX SDK 以 HTTPS POST 回调并输出 `sessiontoken_callback` 日志，插件可凭 state + 签名校验结果。
...
- **FR-010**: `cmd/sessiontoken/main.go` MUST 启动独立 HTTP 服务并注册 `RegisterSessionTokenFlowCreateRoute` 与 `RegisterSessionTokenFlowGetRoute`，默认监听 `:7070`，允许通过配置覆盖端口与 Bearer Token，且 `make sessiontoken`/`go run ./cmd/sessiontoken` 可直接用于本地验证。
- **FR-011**: 文档与配置模板 MUST 提供 `POWERX_SESSION_TOKEN_*` 环境变量映射（`BASE_URL`、`API_TOKEN`、`CALLBACK_URL` 等），以便插件和 MediaX SDK 共享同一服务；`docs/plan/creative/channels.md` 与 README/Quickstart 需同步更新。
- **FR-012**: 系统 MUST 输出 `sessiontoken_metric` 与 `sessiontoken_callback` 观测日志/指标，字段包含 `action/provider/tenant_uuid/flow_id/status/latency_ms/retry`，用于监控 SLA。
...
- **OAuth/AuthManager 触点**: 虽然 OAuth 授权由其他模块负责，但 SessionToken 服务需通过文档指向 AuthManager/SessionManager，说明插件在模拟登录外仍可调用统一授权 URL/回调存储接口，避免重复能力。
- **频道刷新/任务接口**: SessionToken 回调完成后需记录触发 MediaX worker/scheduler 的方式，与 `docs/plan/mediax-sdk.md` 所述 “频道同步 & 任务执行” 的 ingest/任务接口保持一致，文档中需说明依赖关系。
- **配置共享**: `config.yaml`、`config.example.yaml` 与 README 的 Quickstart 需强调会与插件端共享 BaseURL/API Token/CallbackURL，并在样例命令 (`make sessiontoken`) 中展示。
...
- **Operational Reproducibility**: `cmd/sessiontoken/main.go` 与 Makefile targets 必须使用宪章要求的 CLI 结构，`go run ./cmd/sessiontoken`/`make sessiontoken` 需被 Quickstart 引用，确保每位开发能够用统一步骤重现服务行为。
...
- **SC-005**: 通过 `sessiontoken_metric` 与 `sessiontoken_callback` 日志聚合可观测 Flow 创建/查询/回调的 p95 latency、成功率与重试次数，并可由 Quickstart/README 提供的命令在本地重现。
